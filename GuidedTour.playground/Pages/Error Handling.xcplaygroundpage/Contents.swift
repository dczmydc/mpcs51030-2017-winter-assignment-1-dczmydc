...
//my codes from here throwing noTonerError
do {
    let myPrinterResponse = try send(job: 666, toPrinter: "Never Has Toner")
    print(myPrinterResponse)
} catch {
    print(error)
}
...
//for the first catch we need to throw onFire error, for the second we need to throw noToner/outOfPaper error. No errors will reach the third catch.
